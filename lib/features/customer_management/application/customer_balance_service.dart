...
class CustomerBalanceService {
  ...
  Future<List<CustomerBalancePayment>> getPaymentsForCustomer(int customerId) async {
    await AppDatabase.instance.database;
    return AppDatabase.instance.customerBalancePaymentRepository!.getByCustomerId(customerId);
  }

  Future<Customer> updateCustomerStatus({required int customerId, required CustomerStatus status}) async {
    await AppDatabase.instance.database;
  ...
  Future<Customer> recordBalancePayment({
    ...
    await AppDatabase.instance.database;
    final repository = AppDatabase.instance.customerRepository!;
    final paymentRepository = AppDatabase.instance.customerBalancePaymentRepository!;
    ...
    final remainingBalance = customer.currentBalance > amount ? customer.currentBalance - amount : 0.0;
    final updatedCustomer = await repository.update(customer.copyWith(currentBalance: remainingBalance));
